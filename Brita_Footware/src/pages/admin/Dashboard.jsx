...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-6 max-w-full overflow-x-hidden min-w-0">
      {/* Header Banner */}
      <div className="bg-gradient-to-r from-primary via-primary-800 to-secondary rounded-2xl sm:rounded-3xl p-4 sm:p-6 text-white flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shadow-xl min-w-0">
        <div className="min-w-0">
          ...
          <h2 className="font-heading font-black text-xl sm:text-2xl leading-tight">BRITA Business Analytics</h2>
          <p className="text-slate-300 text-xs mt-1 leading-normal">Real-time performance metrics across sales, inventory, and profit.</p>
        </div>

        {/* Time Filters */}
        <div className="flex bg-white/10 backdrop-blur-md rounded-xl p-1 border border-white/20 overflow-x-auto whitespace-nowrap scrollbar-none max-w-full w-full md:w-auto">
        ...
        {kpis.map((kpi, i) => {
          ...
          return (
            <motion.div
              ...
              className="bg-white rounded-2xl p-3.5 sm:p-5 border border-slate-100 shadow-card hover:shadow-card-hover transition-all min-w-0 overflow-hidden"
            >
              <div className="flex items-center justify-between gap-1 mb-1.5 sm:mb-3">
                <span className="text-[11px] sm:text-xs font-semibold text-slate-500 truncate">{kpi.label}</span>
                <div className={`w-7 h-7 sm:w-9 sm:h-9 rounded-lg sm:rounded-xl ${kpi.color} flex items-center justify-center shrink-0`}>
                  <Icon size={16} />
                </div>
              </div>
              <div className="font-heading font-black text-base sm:text-2xl text-primary leading-tight my-1 sm:my-2 truncate" title={kpi.value}>
                {kpi.value}
              </div>
              <div className="flex flex-wrap items-center gap-x-1 gap-y-0.5 text-[10px] sm:text-xs">
                {kpi.isPos ? (
                  <span className="text-green-600 font-bold flex items-center shrink-0"><ArrowUpRight size={12} />{kpi.change}</span>
                ) : (
                  <span className="text-red-500 font-bold flex items-center shrink-0"><ArrowDownRight size={12} />{kpi.change}</span>
                )}
                <span className="text-slate-400 text-[9px] sm:text-[10px] whitespace-nowrap">vs last period</span>
              </div>
            </motion.div>
          );
        })}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 min-w-0">
        ...
        <div className="lg:col-span-2 bg-white rounded-3xl p-4 sm:p-6 border border-slate-100 shadow-card min-w-0 overflow-hidden">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-6">
            <div>
              <h3 className="font-heading font-bold text-base sm:text-lg text-primary">Revenue & Net Profit Trend</h3>
        ...
        {/* Category Pie Chart */}
        <div className="bg-white rounded-3xl p-4 sm:p-6 border border-slate-100 shadow-card flex flex-col min-w-0 overflow-hidden">
          <h3 className="font-heading font-bold text-base sm:text-lg text-primary mb-1">Sales by Category</h3>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-3xl p-4 sm:p-6 border border-slate-100 shadow-card">
          <h3 className="font-heading font-bold text-base sm:text-lg text-primary mb-4">Order Status Breakdown</h3>
          <div className="grid grid-cols-2 gap-2.5 sm:gap-3">
            {Object.entries(orderStatusCounts).map(([status, count]) => {
              ...
              return (
                <div key={status} className={`p-2.5 sm:p-3 rounded-2xl border min-w-0 ${bgMap[status] || "bg-slate-50 text-slate-700"}`}>
                  <p className="text-[10px] sm:text-[11px] font-semibold truncate">{status}</p>
                  <p className="font-heading font-black text-lg sm:text-xl leading-tight mt-0.5">{count}</p>
                </div>
              );
            })}
          </div>
        ...
        <div className="lg:col-span-2 bg-white rounded-3xl p-4 sm:p-6 border border-slate-100 shadow-card overflow-hidden">
          <h3 className="font-heading font-bold text-base sm:text-lg text-primary mb-4">Top Selling Footwear</h3>
                  ...
                  <tr key={p.sku}>
                    ...
                    <td>
                      <div className="flex items-center gap-2.5">
                        <img src={p.image} alt="" className="w-8 h-8 rounded-lg object-cover bg-slate-50 shrink-0" />
                        <div>
                          <p className="font-bold text-primary text-xs whitespace-nowrap">{p.name}</p>
